$(function () {
  ...
  $(".add_email_list_custom_field_to_survey").on("click", function (e) {
    ...
    tinymce
      .get("description")
      .execCommand("mceInsertContent", false, $(this).data("slug"));
  });
});
...
function add_survey_question(type, surveyid) {
  $.post(admin_url + "/public/surveys/add_survey_question", {
    type: type,
    surveyid: surveyid,
    ...
    question_area +=
      '<input type="checkbox" data-question_required="' +
      response.data.questionid +
      '" name="required[]" onchange="update_question(this,\'' +
      type +
      "'," +
      response.data.questionid +
      ')">';
    ...
    question_area +=
      '<label for="' +
      response.data.questionid +
      '" class="control-label display-block">' +
      response.survey_question_string +
      ' <a href="#" onclick="update_question(this,\'' +
      type +
      "'," +
      response.data.questionid +
      '); return false;" class="pull-right update-question-button"><i class="fa fa-refresh text-success question_update"></i></a><a href="#" class="pull-right"><i class="fa fa-remove text-danger" onclick="remove_question_from_database(this,' +
      response.data.questionid +
      '); return false;"></i></a></label>';
    question_area +=
      '<input type="text" onblur="update_question(this,\'' +
      type +
      "'," +
      response.data.questionid +
      ');" data-questionid="' +
      response.data.questionid +
      '" class="form-control questionid">';
    if (type == "textarea") {
      question_area +=
        '<textarea class="form-control mtop20" disabled="disabled" rows="6">' +
        response.survey_question_only_for_preview +
        "</textarea>";
    } else if (type == "checkbox" || type == "radio") {
      question_area += '<div class="row">';
      box_description_icon_class = "fa-plus";
      box_description_function =
        "add_box_description_to_database(this," +
        response.data.questionid +
        "," +
        response.data.boxid +
        "); return false;";
      ...
      question_area +=
        '<a href="#" class="add_remove_action survey_add_more_box" onclick="' +
        box_description_function +
        '"><i class="fa ' +
      ...
      question_area +=
        "<label><input onblur=\"update_question(this,'" +
        type +
        "'," +
        response.data.questionid +
        ');" type="text" data-box-descriptionid="' +
        response.data[0].questionboxdescriptionid +
        '" class="survey_input_box_description"></label>';
      question_area += "</div>";
      question_area += "</div>";
      question_area += "</div>";
      // end box row
      ...
      question_area +=
        '<input type="text" onchange="update_question(this,\'' +
        type +
        "'," +
        response.data.questionid +
        ');" class="form-control mtop20" disabled="disabled" value="' +
        response.survey_question_only_for_preview +
        '">';
    }
    question_area += "</div>";
...
function update_question(question, type, questionid) {
  ...
  var _question = $(question)
    .parents(".question")
    .find('input[data-questionid="' + questionid + '"]')
    .val();
  var _required = $(question)
    .parents(".question")
    .find('input[data-question_required="' + questionid + '"]')
    .prop("checked");
  ...
  if (type == "checkbox" || type == "radio") {
    ...
    $.each(boxes_area, function () {
      var boxdescriptionid = $(this)
        .find("input.survey_input_box_description")
        .data("box-descriptionid");
      var boxdescription = $(this)
        .find("input.survey_input_box_description")
        .val();
  ...
  setTimeout(function () {
    $.post(admin_url + "/public/surveys/update_question", data).done(function (
      response
    ) {
      $(question)
        .parents("li")
        .find("i.question_update")
...
function add_more_boxes(question, boxdescriptionid) {
  ...
  $(box)
    .find("input.survey_input_box_description")
    .attr("data-box-descriptionid", boxdescriptionid);
  $(box).find("input.survey_input_box_description").focus();
  $(box)
    .find(".add_remove_action")
    .attr(
      "onclick",
      "remove_box_description_from_database(this," +
        boxdescriptionid +
        "); return false;"
    );
  update_questions_order();
...
function remove_question_from_database(question, questionid) {
  $.get(
    admin_url + "/public/surveys/remove_question/" + questionid,
    function (response) {
...
function remove_box_description_from_database(
  question,
  questionboxdescriptionid
) {
  $.get(
    admin_url + "/public/surveys/remove_box_description/" + questionboxdescriptionid,
    function (response) {
...
function add_box_description_to_database(question, questionid, boxid) {
  $.get(
    admin_url + "/public/surveys/add_box_description/" + questionid + "/" + boxid,
    function (response) {
...
function update_questions_order() {
  ...
  data = {};
  data.data = update;
  $.post(admin_url + "/public/surveys/update_survey_questions_orders", data);
}
